function [IntLoadAll,stagechangeflag]=ifstd_enriched_old( obj,newmark,id )
...
%        w_trial=lg.Nu*ue;      % trial crack displacement
%        lcr=lg.TractionLaw.Lamdacr
%        if w_trial
       lg.Traction=lg.TractionO+lg.Tangent_coh*lg.Nu*Due;
       % currently the crack opening update is at the end of each increment
       % as of modified newtonraphson scheme, error may occur if the
       % separation stage changes from the trial to updated. More efforts
       % are necessary to implement the right update of traction. For now,
       % only a warning is issued when such change is monitored. 04042019
       if max(lg.Traction)>lg.TractionLaw.PeakTraction*1.01
           % this 1.01 ratio allows the stage transition
           % warning('The traction updating may be wrong because of stage change');
           if max(lg.TractionO)<lg.TractionLaw.PeakTraction*0.99
           ...
           % times when the TractionO is around the peaktraction.
               ...
               lg.Traction=lg.TractionLaw.PeakTraction;
               stagechangeflag=false;
           end
       else
           stagechangeflag=false;
       end
       % WRONG UPDATION FOR UNLOADING.(NOT FIXED) 03222019
       % To enforce the zero traction after complete separation 03172019
       if lg.TractionLaw.Separated==1
           lg.Traction=[0;0];
       end
%        Jacob.F_coh_i=Jacob.F_coh_old+lg.H*lg.Nu'*lg.Traction*lg.Ds;
    ...
    if any(any(Jacob.Kc))==0
        Jacob.F_coh_i=zeros(size(Jacob.F_coh_old));
    end
