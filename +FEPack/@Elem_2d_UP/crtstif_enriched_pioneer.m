function crtstif_enriched_pioneer( obj, newmark, id )
...
global  Delastic Density Biot_alpha kmat Cstar Kf muf;
if obj.Enrich(id)==0
...
%         % Nuenr and Benr are not used in the current version 03282019
        ...
        % crack.(04242019)
        kd=wd^3/12/muf/1.24;                        % f=1.24 in [1.04,1.65]
...
Jpsus=a1*JMat.Qusps';
Jpsue=a1*transpose(JMat.Queps);
% IMPORTANT ERROR: SIGN OF HINTPSPS, SINTPSPS. (FIXED ON 04022019)
Jpsps=JMat.Hpsps+a1p*(JMat.Spsps);
% IMPORTANT ERROR: SIGN OF HINTPSPE, SINTPSPE. (FIXED ON 04022019)
Jpspe=JMat.Hpspe+a1p*(JMat.Spspe);
Jpeus=a1*JMat.Quspe';
Jpeue=a1*transpose(JMat.Quepe+Qintuepe+Qintueps);
% IMPORTANT ERROR: SIGN OF HINTPEPS, SINTPEPS. (FIXED ON 04022019)
Jpeps=JMat.Hpeps-Hintpeps-Hintpsps+a1p*(JMat.Speps-Sintpeps-Sintpsps);
% IMPORTANT ERROR: SIGN OF HINTPEPE, SINTPEPE. (FIXED ON 04022019)
Jpepe=JMat.Hpepe-Hintpepe-Hintpspe+a1p*(JMat.Spepe-Sintpepe-Sintpspe);
